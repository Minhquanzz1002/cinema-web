...
const PromotionPage = () => {
    ...
    const { data: responseData } = useAllActors();
    ...
    const columns = React.useMemo<ColumnDef<Actor>[]>(
        () => [
            {
                accessorKey: 'id',
                header: () => (
                ...
                cell: ({ row }) => {
                    return (
                        <div className="w-14 h-14 relative rounded shadow overflow-hidden">
                            {
                                row.original.image ? <Image src={row.original.image} alt={row.original.name} fill
                                                            sizes="(max-width: 768px) 100vw, (max-width: 1200px) 50vw, 33vw"
                                                            priority className="rounded-md object-cover" /> :
    ...
    const handleExportExcel = () => {
        exportToExcel<Actor>(actors, [], 'actors.xlsx');
    };
    ...
    return (
        <>
            <div className="mt-3">
                ...
                <Table<Actor> data={actors} columns={columns} currentPage={currentPage} totalPages={totalPages}
